...
# 1. 난수 생성을 위한 시드(Seed) 고정
# 매번 실행할 때마다 똑같은 무작위 데이터가 생성되도록 함
np.random.seed(42)
random.seed(42)
...
korean_genres = [
    "액션", "드라마", "코미디", "로맨스", "스릴러",
    "판타지", "공포", "애니메이션", "가족", "다큐멘터리"
]

movie_titles = [
    "국가대표", "탐정", "괴물", "도둑들", "공조", "검사외전",
    "내 친구의 집은 어디인가", "군함도", "암살", "베테랑", "터널",
    "골든슬럼버", "국가대표 2", "대호", "밀정", "특전사령부",
    "범죄도시", "마약왕", "신의 한수", "나쁜 녀석들", "스윗홈",
    "나이브스 아웃", "악인전", "혹성탈출", "전쟁 영화", "한산",
    "명량", "적의", "공중도둑", "신세계", "연평해전", "타짜",
    "혜화역 1 번 출구", "원더", "럭키", "캐스팅", "살인", "계획",
    "복수는 나의 것", "엄마는 외계인", "아저씨", "곡성",
    "살인자의 기억법", "라이언 고슬링", "미션임파서블", "분노의 질주",
    "스타워즈", "어벤져스", "반도", "스파이더맨", "배트맨", "조커",
    "더 라이트하우스", "컨테이전", "블랙팬서", "캡틴마블",
    "어벤져스엔드게임"
]


# 2. 영화 데이터셋을 무작위로 만드는 함수
# 기본값: 1000개 영화
def generate_movie_dataset(n_movies=1000):
    # 생성된 데이터들을 저장할 빈 딕셔너리
    # 각 key는 데이터셋의 열 이름이 되고, value는 해당 열에 들어갈 값들의 리스트가 됨
    data = {
        "title": [],
        "genre": [],
        "budget": [],
    ...
    # 지정한 영화 개수(n_movies)만큼 반복하며 한 줄씩 데이터를 만듦
    for i in range(n_movies):
        # [영화 제목 설정]
        # 영화 제목 목록을 순환하며, 중복될 경우 뒤에 '2', '3' 같은 번호를 붙임
        ...
        # [상영시간(runtime) 설정]
        # 평균 115분, 표준편차 20분인 정규분포로 생성 후 90~180분 사이로 제한
        runtime = max(90, min(180, int(np.random.normal(115, 20))))

        # [개봉월(release_month) 설정]
        # 1월부터 12월 중 무작위 정수 하나 선택
        release_month = random.randint(1, 12)

        # [평점(vote_average) 설정]
        # 평균 6.5, 표준편차 1.5인 정규분포로 생성 후 1~10점 사이로 제한
        vote_average = round(max(1.0, min(10.0, np.random.normal(6.5, 1.5))), 1)

        # [제작사 규모 설정]
        # 1(소형), 2(중형), 3(대형) 중 하나를 무작위 선택
        production_company_size = random.choice([1, 2, 3])

        # [마케팅 점수 설정]
        # 0~100점 사이로 제한된 인지도 점수 생성
        marketing_score = round(max(0, min(100, np.random.normal(50, 25))), 1)

        # [속편 여부(sequel_flag) 설정]
        # 25% 확률로 속편(1), 아니면 0
        sequel_flag = 1 if random.random() < 0.25 else 0

        # [성공 공식 계산]
        # 각 요소에 가중치를 곱해 기본 성공 점수 계산
        success_score = (
            (budget / 500) * 0.25
            + (director_popularity / 100) * 0.25
            + (cast_popularity / 100) * 0.20
            + (vote_average / 10) * 0.15
            + (marketing_score / 100) * 0.15
        )

        # 성공 점수에 약간의 무작위 노이즈를 섞어서
        # 최종 성공 확률을 0.05~0.95 사이로 만듦
        success_probability = min(
            0.95,
            max(0.05, success_score + np.random.normal(0, 0.1)),
        )

        # 속편이거나 성수기(7, 8, 12월)에 개봉하면 성공 확률을 가산
        if sequel_flag == 1:
        ...
        # 최종 계산된 확률에 따라 천만 관객 돌파 여부(1 또는 0)를 결정
        hit_10M = 1 if random.random() < success_probability else 0

        # [관객수 및 매출액(revenue) 결정]
        # 천만 돌파 여부에 따라 관객수를 무작위로 부여
        if hit_10M == 1:
            audience = np.random.normal(loc=15_000_000, scale=5_000_000)
            audience = max(10_000_000, audience)  # 천만 돌파 영화는 무조건 1000만 명 이상
        else:
            audience = np.random.normal(loc=5_000_000, scale=2_000_000)
            audience = max(100_000, min(9_999_999, audience))  # 실패 영화는 10만~999만 명 사이

        # 매출액 계산
        # 대략 [관객수 * 12,000원]을 계산한 후 백만 원 단위로 변환
        revenue = round(audience * 12 / 1_000_000, 1)

        # 생성된 한 편의 영화 데이터를 딕셔너리의 리스트들에 추가
    ...
    # 완성된 딕셔너리 데이터를 판다스의 DataFrame 구조로 만들어 반환
    return pd.DataFrame(data)


# 3. 이상치(Outlier) 전처리 함수
def preprocess_data(df):
    # 원본 데이터프레임을 직접 수정하지 않도록 복사본 생성
    df = df.copy()

    # 데이터프레임에서 숫자형 열들만 골라냄
    numeric_columns = df.select_dtypes(include=[np.number]).columns

    # 각 숫자형 열들을 돌며 IQR 기준을 벗어나는 극단적인 값들을 한계값으로 조절
...
if __name__ == "__main__":
    # 1) 1000개의 가상 영화 데이터를 생성
    df = generate_movie_dataset(1000)

    # 2) 극단적인 수치 데이터를 다듬는 전처리 수행
    ...
    # 4) 생성된 데이터의 상위 5개 줄 출력
    print("--- 데이터 상위 5개 행 보기 ---")
    print(df.head())

    # 5) 천만 영화(1)와 실패 영화(0)의 개수 출력
    print("\n--- 천만 돌파 여부 빈도수 구하기 ---")
    print(df["hit_10M"].value_counts().sort_index())

    # 6) 전체 데이터 중 천만 영화가 차지하는 비중 출력
